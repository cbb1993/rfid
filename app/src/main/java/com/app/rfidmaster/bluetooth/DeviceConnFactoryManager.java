...
public class DeviceConnFactoryManager {
    ...
    public boolean openPort() {
        ...
        Log.e(TAG,"isOpenPort = " + isOpenPort);
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case Constant.abnormal_Disconnection://异常断开连接
                    ToastUtil.show(App.getContext().getString(R.string.str_disconnect));
                    break;
                case DEFAUIT_COMMAND://默认模式
                    ToastUtil.show(App.getContext().getString(R.string.default_mode));
                    break;
                case READ_DATA:
                    ...
                    if (sendCommand == esc) {
                        ...
                        if (currentPrinterCommand == null) {
                            currentPrinterCommand = PrinterCommand.ESC;
                            sendStateBroadcast(CONN_STATE_CONNECTED);
                            ToastUtil.show(App.getContext().getString(R.string.str_escmode));
                        } else {//查询打印机状态
                            if (result == 0) {//打印机状态查询
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            sendStateBroadcast(CONN_STATE_CONNECTED);
                            ToastUtil.show(App.getContext().getString(R.string.str_tscmode));
                        } else {
                            if (cnt == 1) {//查询打印机实时状态
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            sendStateBroadcast(CONN_STATE_CONNECTED);
                            ToastUtil.show(App.getContext().getString(R.string.str_cpclmode));
                        }else {
                            if (cnt == 1) {
                                System.out.println(App.getContext().getString(R.string.str_state) + status);
